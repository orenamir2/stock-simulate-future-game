...
- Create exactly 20 three-year outcome representatives. Provide complete factorStates for each scenario; different product outcomes may share the same broad factor states. Use distinct joint company outcomes and model correlated events together, rather than counting overlapping events as exclusive scenarios. The server sorts derived prices into contiguous, non-overlapping terminal-price buckets that collectively cover all non-negative prices.
...
- For each scenario provide explicit valuationInputs. forecast revenue is server-derived from revenueBridge; revenueCagrPct is retained for legacy compatibility and ignored when a bridge is supplied. For enterprise-value-multiple use revenue, EBIT or free cash flow; server calculates EV = metric × multiple and equity = EV + net cash. For equity-value-multiple use net income or book value; server calculates equity = metric × multiple. For NAV use NAV or book value. The server then converts reporting currency to trading currency and divides by diluted shares.
- Provide revenueBridge for every scenario: 1–12 disjoint named products or reported segments, including an explicit other/undisclosed residual where needed. Use the same baseline partition in all scenarios; baselineRevenue sums to company baseline revenue within 0.1%, in its reporting currency and scale. Never invent an undisclosed product revenue split.
- For each product supply volumeRatio and priceRatio (year-three annual volume and net realized price divided by baseline; 1 means unchanged), plus newAnnualRevenue (incremental year-three annual revenue outside the baseline cohort, in baseline scale). The server sums baselineRevenue × volumeRatio × priceRatio + newAnnualRevenue. Ratios describe the entire three-year change, not annual growth. Do not count new customers in both volumeRatio and newAnnualRevenue. A zero-revenue pipeline can have baselineRevenue=0 and positive newAnnualRevenue after commercialization.
- Each bridge row must name a specific event or continuation assumption, its expected timing, an observable leadingIndicator that would falsify it, and sourceIds. In event text distinguish disclosed facts from your estimates; explain how timing, capacity, adoption, pricing and cannibalization produce the numeric inputs. Cite evidence for the mechanism, not as proof of an invented forecast. If product economics are undisclosed use reported segments and explicitly explain the limitation.
- Choose mechanisms relevant to this company: software renewal cohorts, seat/usage contraction and new workloads; semiconductor design wins, qualification, foundry/packaging yield, export eligibility and generation transitions; medicines trial results, approval, reimbursement, patent expiry and generic entry; consumer products replacement cycles, channel inventory and recalls; industrial businesses order cancellation, backlog conversion, commissioning and warranty costs. Name actual products, customers or dependencies only when evidenced. Do not force irrelevant sector events into the analysis.
- Model connected events jointly: a launch delay changes year-three availability; cannibalization reduces the incumbent row; a customer loss affects exposed revenue, not all company sales. Reflect resulting costs, reinvestment and financing consistently in margin/netCash/share assumptions. These terminal bridges do not establish empirical event probabilities.
