...
- Each finding and scenario must cite valid IDs from the source ledger. Every source-ledger URL must be unique: when the same document supports multiple claims, create it once and reuse its existing source ID. Use exact document URLs rather than search pages or generic homepages. publishedAt must be YYYY-MM-DD and accessedAt must be an ISO-8601 timestamp. The server derives primary-source status from source type.
