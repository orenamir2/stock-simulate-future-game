...
New research runs include a product/segment revenue bridge for each scenario. The server calculates year-three annual revenue as the sum of `baselineRevenue × volumeRatio × priceRatio + newAnnualRevenue`. Each row identifies a company-specific event, timing, observable indicator and evidence. Existing-product contraction and cannibalization reduce the incumbent row; new commercialization can generate revenue even from a zero baseline. Baseline rows must reconcile to company revenue within 0.1%. Undisclosed product splits must remain explicitly unresolved at the reported-segment level. The UI exposes each bridge. Older saved analyses without bridges retain the aggregate-CAGR calculation; with a bridge, the legacy `revenueCagrPct` input is ignored. Timing is currently explanatory, not an annual cash-flow simulation.
