...
Research runs use live web search, low reasoning effort, an 8-minute inactivity timeout, and a 25-minute end-to-end safety timeout by default. The research prompt also asks the agent to use a bounded set of focused searches and return once the minimum evidence standard is met. The five-minute margin keeps process cleanup, validation, persistence, and response delivery inside the 30-minute user-facing limit. Any Codex output resets the inactivity timer. Terminal Codex JSONL error events fail immediately, and cancellation or timeout kills the complete Unix process group so descendants cannot continue as orphans. Set `CODEX_REASONING_EFFORT`, `CODEX_IDLE_TIMEOUT_MS`, or `CODEX_TIMEOUT_MS` to tune those operational limits; Kubernetes declares the same defaults in `k8s/deployment.yaml`.
